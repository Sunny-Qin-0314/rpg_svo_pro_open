...
FrameHandlerMonoEvent::FrameHandlerMonoEvent(
...
void FrameHandlerMonoEvent::addImages(
    const cv::Mat& imgMono,
    const cv::Mat& imgEvent,
    const uint64_t timestamp)
{
  addImageBundle({imgMono, imgEvent}, timestamp);
}


// TODO: yuqing
UpdateResult FrameHandlerMonoEvent::processFirstFrame()
{
...
UpdateResult FrameHandlerMonoEvent::processFrame()
{
...
UpdateResult FrameHandlerMonoEvent::relocalizeFrame(
...
void FrameHandlerMonoEvent::resetAll()
{
  if(bundle_adjustment_type_==BundleAdjustmentType::kCeres)
  {
...
FramePtr FrameHandlerMonoEvent::lastFrame() const
{
  return (last_frames_ == nullptr) ? nullptr : last_frames_->at(0);
}

const FramePtr& FrameHandlerMonoEvent::newFrame() const
{
  return new_frames_->frames_[0];
}

const FramePtr& FrameHandlerMonoEvent::lastFrameUnsafe() const
{
  return last_frames_->frames_[0];
}

bool FrameHandlerMonoEvent::haveLastFrame() const
{
